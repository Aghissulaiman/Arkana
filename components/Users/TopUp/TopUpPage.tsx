...
export default function TopUpPage() {
  ...
  const fetchBalance = async () => {
    ...
    try {
      ...
      if (!user) return;
      const { data } = await supabase.from("profiles").select("balance_points").eq("user_id", user.id).single();
  ...
  const addPointsToUser = async (userId: string, pointsToAdd: number) => {
    ...
    if (rpcError) {
      console.log("RPC error, trying manual update:", rpcError);
      ...
      const newBalance = (profile?.balance_points || 0) + pointsToAdd;
    ...
    return newProfile?.balance_points || 0;
  };
      ...
      if (snapLoaded && window.snap) {
        window.snap.pay(data.token, {
          onSuccess: async (result: any) => {
            ...
            try {
              // Tambah poin ke user
              const newBalance = await addPointsToUser(user.id, points);
              setSuccessPoints(points);
              setSuccessBalance(newBalance);
              setStep("success");
              toast.success(`+${points.toLocaleString()} poin berhasil ditambahkan!`);
            } catch (err) {
              console.error("Error adding points:", err);
              toast.error("Pembayaran sukses tapi gagal menambah poin. Hubungi admin.");
            }
            setSubmitting(false);
          },
  ...
  if (step === "success") {
    return (
      <div className="max-w-md mx-auto px-4 py-12 text-center">
        ...
        <p className="text-gray-500 mb-4">Poin Anda telah bertambah.</p>
        <div className="bg-gray-50 rounded-xl p-4 mb-6 text-left space-y-2">
          <div className="flex justify-between text-sm">
            <span className="text-gray-500">Poin yang Didapat</span>
            <span className="font-semibold text-primary">+{successPoints.toLocaleString()}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-gray-500">Saldo Sekarang</span>
            <span className="font-semibold">{successBalance.toLocaleString()} poin</span>
          </div>
        </div>
        <button
          onClick={() => {
    ...
    return (
      <div className="max-w-md mx-auto px-4 py-6">
        ...
        <div className="bg-gradient-to-br from-primary to-primary/80 rounded-2xl p-5 text-white mb-6">
          ...
          <p className="text-white/70 text-sm">= +{getPointsEarned().toLocaleString()} poin</p>
        </div>
        ...
        <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 mb-6">
          <div className="flex items-start gap-2">
            ...
            <div className="text-xs text-amber-700">
              ...
              <p>4. Klik "Bayar" → poin akan langsung bertambah!</p>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Tambah saldo poin untuk ditukarkan dengan reward</p>
      </div>
      ...
      <div className="bg-gradient-to-br from-primary to-primary/80 rounded-2xl p-6 text-white">
        ...
        <p className="text-white/60 text-sm mt-1">≈ Rp {(balance * RATE).toLocaleString("id-ID")}</p>
      </div>

      <div className="bg-primary/5 border border-primary/15 rounded-xl p-3 flex items-center gap-3 text-sm">
        <Info className="w-4 h-4 text-primary shrink-0" />
        <p className="text-primary/80"><strong>Kurs:</strong> Rp {RATE.toLocaleString()} = 1 poin</p>
      </div>
