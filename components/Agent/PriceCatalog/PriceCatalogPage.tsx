...
export default function AgentPriceCatalogPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (wasteTypes.length === 0) {
        toast.warning("Anda belum memilih jenis sampah yang diterima.");
        setLoading(false);
        return;
      }
      ...
      wasteTypes.forEach((wasteType: string) => {
        ...
        if (agentPrice) {
          ...
          merged.push({ waste_type: wasteType, price_per_kg: 0, is_custom: false });
        }
      });
  ...
  const handleSaveEdit = async (wasteType: string) => {
    ...
    setSaving(true);
    try {
      const existing = prices.find((p) => p.waste_type === wasteType);
      const isCustom = existing?.is_custom;

      if (isCustom) {
        const { error } = await supabase
          .from("price_catalog")
          .update({ price_per_kg: val })
          .eq("id", existing?.id);
  ...
  const handleResetToGlobal = async (wasteType: string) => {
    const existing = prices.find((p) => p.waste_type === wasteType);
    if (!existing?.id || !existing?.is_custom) return;

    if (!confirm("Reset ke harga global? Harga khusus agent akan dihapus.")) return;
    ...
    try {
      ...
      if (error) throw error;
      toast.success("Kembali ke harga global");
      await fetchData();
    } catch (err: any) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="max-w-md mx-auto text-center">
          ...
          <button onClick={() => (window.location.href = "/agent/dashboard")} className="px-6 py-2 bg-primary text-white rounded-lg">
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <button onClick={fetchData} className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors text-sm">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center text-xl ${wasteInfo.color}`}>
                      {wasteInfo.emoji}
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      {!isEditing ? (
                        <>
                          ...
                          {isCustom && (
                            <button
                              onClick={() => handleResetToGlobal(priceData.waste_type)}
                              disabled={saving}
        ...
        <div className="bg-gray-100 rounded-xl p-4 text-center">
          ...
          <p className="text-xs text-gray-500">
            Klik edit ✏️ untuk mengatur harga. Harga khusus akan mengesampingkan harga global.
            <br />
            Klik reset 🔄 untuk kembali ke harga global.
          </p>
        </div>
      </div>
    </div>
